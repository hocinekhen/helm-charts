...
if ! ip link show $UL_IFACE; then
    s1u_mac=$(ip addr show dev s1u-net2 | awk '$1=="link/ether"{print $2}')
...
if ! ip link show $DL_IFACE; then
    sgi_mac=$(ip addr show dev sgi-net2 | awk '$1=="link/ether"{print $2}')
...
if ! ip addr show $UL_IFACE | grep inet; then
    s1u_ip=$(ip addr show s1u-net2 | grep inet | grep -v inet6 | awk '{print $2}')
    ip addr add $s1u_ip dev $UL_IFACE
fi
if ! ip addr show $DL_IFACE | grep inet; then
    sgi_ip=$(ip addr show sgi-net2 | grep inet | grep -v inet6 | awk '{print $2}')
